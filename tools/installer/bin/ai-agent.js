...
╚═══════════════════════════════════════╝
  ...
  if (components.includes('javascript')) {
    await copyDirectory(
      ...
      path.join(installDir, '.ai-agent', 'rules', 'javascript'),
      'JavaScript Rules'
    );
  }
  ...
  if (components.includes('js-memory')) {
    await copyDirectory(
      ...
      path.join(installDir, '.ai-agent', 'memory', 'javascript'),
      'JavaScript Memory'
    );
  }
  ...
  if (components.includes('sql')) {
    await copyDirectory(
      ...
      path.join(installDir, '.ai-agent', 'rules', 'sql'),
      'SQL Rules'
    );
  }
  ...
  if (components.includes('sql-memory')) {
    await copyDirectory(
      ...
      path.join(installDir, '.ai-agent', 'memory', 'sql'),
      'SQL Memory'
    );
  }
...
${components.includes('javascript') ? '- JavaScript Rules & Guidelines (`.ai-agent/rules/javascript/`)' : ''}
${components.includes('sql') ? '- SQL Rules & Guidelines (`.ai-agent/rules/sql/`)' : ''}
${components.includes('js-memory') ? '- JavaScript Memory Context (`.ai-agent/memory/javascript/`)' : ''}
${components.includes('sql-memory') ? '- SQL Memory Context (`.ai-agent/memory/sql/`)' : ''}
...
\`\`\`bash
npx ai-agent install
\`\`\`
`;

  await fs.writeFile(
    path.join(installDir, '.ai-agent', 'README.md'),
    instructionsContent
  );
  ...
  if (await fs.pathExists(gitignorePath)) {
    ...
    if (!gitignore.includes('.ai-agent')) {
  ...
  console.log(chalk.cyan('📝 Framework installed in:'), chalk.bold(path.join(installDir, '.ai-agent')));
  console.log(chalk.cyan('📖 Read the README:'), chalk.bold(path.join(installDir, '.ai-agent', 'README.md')));
}
...
async function listAvailable() {
  ...
  console.log(chalk.dim('Install with:'), chalk.cyan('npx ai-agent install\n'));
}
